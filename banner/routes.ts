...
export default async function bannerRoutes(fastify: FastifyInstance, opts: FastifyPluginOptions) {
    ...
    fastify.post('/getBanners', { preHandler: [adminAuthValidationPreHandler, validation(listBannerValidate)] }, handler.listbannerhandler);
    ...
    fastify.get('/:id',{ preHandler: [adminAuthValidationPreHandler] },handler.singleBannerHandler);
    fastify.delete('/delete-banner/:id',{ preHandler: [adminAuthValidationPreHandler] },handler.deleteBannerHandler);
}
